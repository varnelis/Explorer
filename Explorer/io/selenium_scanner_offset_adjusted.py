...
class SeleniumScanner:
    ...
    def scroll_page(cls):
        ...
        new_height = cls.get_page_height()
        return new_height
    ...
    def get_page_height(cls):
        return cls.driver.execute_script("return document.body.getBoundingClientRect().top")
    ...
    @classmethod
    def load_bbox(cls, rescale=False):
        ...
        for e in elements:
            ...
            cls.bbox.append(
                (
                    pos["x"],
                    pos["y"],# + (rescale == True) * offset,
                    (pos["x"] + dim["width"]),
                    (pos["y"] + dim["height"]), #+ (rescale == True) * offset 
                    e,
                )
            )
