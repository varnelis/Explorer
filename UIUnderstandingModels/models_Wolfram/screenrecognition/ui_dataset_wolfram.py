...
class WolframUIDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        root="../../downloads/wolfram/screenshots",
        class_dict_path="../../metadata/screenrecognition/class_map_wolfram_manual.json",
        id_list_path="../../metadata/screenrecognition/train_ids_wolfram.json",
        track_uuid=False,
    ):
    ...
    def __getitem__(self, idx):
        def return_next():  # for debugging
            return WolframUIDataset.__getitem__(self, idx + 1)
...
class WolframUIOneHotLabelDataset(WolframUIDataset):
    def __getitem__(self, idx):
        img, res_dict = super(WolframUIOneHotLabelDataset, self).__getitem__(idx)
        num_classes = 2
...
class WolframUIDataModule(pl.LightningDataModule):
    def __init__(self, batch_size=32, num_workers=2, one_hot_labels=True, subset_size=-1, rand_seed=0):
        ''' Dataloader for Wolfram data at given batch size. 2 Workers works good on JADE. If subset_size=-1, all; else subset. '''
        super(WolframUIDataModule, self).__init__()
        ...
        if one_hot_labels:
            self.train_dataset = WolframUIOneHotLabelDataset(
                id_list_path="../../metadata/screenrecognition/train_ids_wolfram.json"
            )
            self.val_dataset = WolframUIOneHotLabelDataset(
                id_list_path="../../metadata/screenrecognition/val_ids_wolfram.json"
            )
            self.test_dataset = WolframUIOneHotLabelDataset(
                id_list_path="../../metadata/screenrecognition/test_ids_wolfram.json"
            )
        else:
            self.train_dataset = WolframUIDataset(
                id_list_path="../../metadata/screenrecognition/train_ids_wolfram.json"
            )
            self.val_dataset = WolframUIDataset(
                id_list_path="../../metadata/screenrecognition/val_ids_wolfram.json"
            )
            self.test_dataset = WolframUIDataset(
                id_list_path="../../metadata/screenrecognition/test_ids_wolfram.json"
            )

    def train_dataloader(self):
        if self.subset_size != -1:
            print("\nWolframUIDataModule: SUBSET SIZE != -1, RAND DATALOADER SUBSIZE\n")
