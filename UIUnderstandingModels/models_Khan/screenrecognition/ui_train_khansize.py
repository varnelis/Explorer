if __name__ == "__main__":
    ...
    logger = TensorBoardLogger(ARTIFACT_DIR, version=f'version-subsize{subsize}')
    
    data = KhanUIDataModule(batch_size=batch_size, subset_size=subsize)
    ...
    model = UIElementDetector(num_classes=2, lr=lr, val_weights=class_weights, test_weights=class_weights)
    ...
    checkpoint_callback2 = ModelCheckpoint(dirpath=ARTIFACT_DIR, filename=f"screenrecognition_best_size{subsize}",monitor='val_mAP', mode="max", save_top_k=1)
