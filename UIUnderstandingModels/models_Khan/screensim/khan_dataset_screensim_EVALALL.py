...
class KhanSimilarityDataset(torch.utils.data.Dataset):
    def __init__(self, 
                 ...
                 uuid2ocr_file="../../metadata/screensim/ocr/uuid2ocr_base.json",
                 uuid2int_file="../../metadata/screensim/ocr/uuid2int.json",
                 int2uuid_file="../../metadata/screensim/ocr/int2uuid.json",
                 ):
        ...
        all_uuids = set(list(self.uuid2ocr.keys()))
        self.uuid2int = {k:v for v,k in enumerate(all_uuids)}
        self.int2uuid = {k:v for k,v in enumerate(all_uuids)}
        with open(uuid2int_file, "w") as f:
            json.dump(self.uuid2int, f)
        with open(int2uuid_file, "w") as f:
            json.dump(self.int2uuid, f)
        self.uuid2int_file = uuid2int_file
    ...
    def sample_same_screen(self):
        try:
            ...
            img1_path = os.path.join(self.root_dir, ''.join(random_uuid_1.split('-')) + ".png")
            img2_path = os.path.join(self.root_dir, ''.join(random_uuid_2.split('-')) + ".png")
            ...
            # UUIDs for TF-IDF
            uuid_int1 = self.uuid2int[''.join(random_uuid_1.split('-'))]
            uuid_int2 = self.uuid2int[''.join(random_uuid_2.split('-'))]
    ...
    def sample_different_screen(self): # different domain
        try:
            ...
            img1_path = os.path.join(self.root_dir, ''.join(uuid1.split('-')) + ".png")
            img2_path = os.path.join(self.root_dir, ''.join(uuid2.split('-')) + ".png")
            ...
            # UUIDs for TF-IDF
            uuid_int1 = self.uuid2int[''.join(uuid1.split('-'))]
            uuid_int2 = self.uuid2int[''.join(uuid2.split('-'))]
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            return label, encodings_img1_upsample, encodings_img2_upsample, center_img_1_upsample, center_img_2_upsample, uuid_int1, uuid_int2#, ocr1, ocr2

        except Exception as e:
            print('failed ', idx, str(e))
            return return_next()

class KhanSimilarityDataModule(pl.LightningDataModule):
    def __init__(self, batch_size=16, num_workers=2, split_file="../../metadata/screensim/train_split_khan_screensim.json"):
        super(KhanSimilarityDataModule, self).__init__()
        self.batch_size = batch_size
        self.num_workers = num_workers
        self.split_file = split_file
        
        self.train_dataset = KhanSimilarityDataset(split_file=split_file)
        self.val_dataset = KhanSimilarityDataset(split_file="../../metadata/screensim/val_split_khan_screensim.json")
        self.test_dataset = KhanSimilarityDataset(split_file="../../metadata/screensim/test_split_khan_screensim.json")
