...
if __name__ == "__main__":
    files = '../../metadata/screenrecognition/test_ids_wikipedia.json'
    ...
    for uuidset in tqdm(uuids):
        uuid = uuidset["uuid"]
        img = f'../../downloads/wikipedia/screenshots/{uuid}.png'
        bboxfile = f'../../downloads/wikipedia/annotations/{uuid}.json'
        with open(bboxfile,'r') as fp:
            bboxes = json.load(fp)
        bboxes = bboxes["clickable"]
        imgdrawbbox = draw_bbox(bboxes,img)
        imgdrawbbox.save(f'../../downloads/wikipedia/bbox_test_screenshots/{uuid}.png')