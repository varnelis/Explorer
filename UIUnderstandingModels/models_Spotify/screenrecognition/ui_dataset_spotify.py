...
class SpotifyUIDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        root="../../downloads/spotify/screenshots",
        class_dict_path="../../metadata/screenrecognition/class_map_spotify_manual.json",
        id_list_path="../../metadata/screenrecognition/train_ids_spotify.json",
        track_uuid=False,
    ):
    ...
    def __getitem__(self, idx):
        def return_next():  # for debugging
            return SpotifyUIDataset.__getitem__(self, idx + 1)
...
class SpotifyUIOneHotLabelDataset(SpotifyUIDataset):
    def __getitem__(self, idx):
        img, res_dict = super(SpotifyUIOneHotLabelDataset, self).__getitem__(idx)
        num_classes = 2
...
class SpotifyUIDataModule(pl.LightningDataModule):
    def __init__(self, batch_size=32, num_workers=2, one_hot_labels=True, subset_size=-1, rand_seed=0):
        ''' Dataloader for Spotify data at given batch size. 2 Workers works good on JADE. If subset_size=-1, all; else subset. '''
        super(SpotifyUIDataModule, self).__init__()
        ...
        if one_hot_labels:
            self.train_dataset = SpotifyUIOneHotLabelDataset(
                id_list_path="../../metadata/screenrecognition/train_ids_spotify.json"
            )
            self.val_dataset = SpotifyUIOneHotLabelDataset(
                id_list_path="../../metadata/screenrecognition/val_ids_spotify.json"
            )
            self.test_dataset = SpotifyUIOneHotLabelDataset(
                id_list_path="../../metadata/screenrecognition/test_ids_spotify.json"
            )
        else:
            self.train_dataset = SpotifyUIDataset(
                id_list_path="../../metadata/screenrecognition/train_ids_spotify.json"
            )
            self.val_dataset = SpotifyUIDataset(
                id_list_path="../../metadata/screenrecognition/val_ids_spotify.json"
            )
            self.test_dataset = SpotifyUIDataset(
                id_list_path="../../metadata/screenrecognition/test_ids_spotify.json"
            )

    def train_dataloader(self):
        if self.subset_size != -1:
            print("\nSpotifyUIDataModule: SUBSET SIZE != -1, RAND DATALOADER SUBSIZE\n")
