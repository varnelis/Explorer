if __name__ == "__main__":
    ...
    ARTIFACT_DIR = "./checkpoints_screenrecognition_spotify"
    ...
    from ui_dataset_spotify import *
    ...
    checkpoint_callback2 = ModelCheckpoint(dirpath=ARTIFACT_DIR, filename=f"screenrecognition_best_res{min_size}",monitor='val_mAP', mode="max", save_top_k=1)
    ...
    for seed in range(1):
        ...
        web7kbal_ckpt = os.path.join(ARTIFACT_DIR, 'screenrecognition-web7kbal.ckpt') # load from web7kbal
        vins_ckpt = os.path.join(ARTIFACT_DIR, 'screenrecognition-vins.ckpt') # load from vins
        if os.path.exists(web7kbal_ckpt) or os.path.exists(vins_ckpt):
            model = UIElementDetector.load_from_checkpoint(vins_ckpt, val_weights=class_weights, test_weights=class_weights, lr=lr)
            mod = model.model.head.classification_head
            model.model.head.classification_head.cls_logits = torch.nn.Conv2d(mod.cls_logits.in_channels, mod.num_anchors * FINETUNE_CLASSES, kernel_size=3, stride=1, padding=1)
            model.model.head.classification_head.num_classes = FINETUNE_CLASSES
            print(f'\nMODEL LOADED from {vins_ckpt}\n')
        else:
            model = UIElementDetector()

        model.hparams.num_classes = FINETUNE_CLASSES
        model.hparams.lr = lr
        
        #spotify_ckpt = os.path.join(ARTIFACT_DIR, 'screenrecognition_best_res720.ckpt')
        #model = UIElementDetector.load_from_checkpoint(spotify_ckpt, val_weights=class_weights, test_weights=class_weights, lr=lr)
        ...
        data = SpotifyUIDataModule(batch_size=batch_size, subset_size=subsize, rand_seed=seed)
