if __name__ == "__main__":
    ...
    ARTIFACT_DIR = "checkpoints_screensim_spotify_noocr_EVAL"
    ...
    from spotify_dataset_screensim import *
    from spotify_dataset_screensim_EVALALL import *
    from ui_models_spotify_centerness_FC_EVALALL import *
    ...
    data_trainrand = SpotifySimilarityDataModule(batch_size=batch_size)
    data_EVALALL = SpotifySimilarityDataModule_EVAL(batch_size=batch_size)
    ...
    checkpoint_callback2 = ModelCheckpoint(dirpath=ARTIFACT_DIR, 
                                           filename= "screensim-noocr-spotify-best-EVAL", 
                                           save_top_k=1, 
                                           every_n_train_steps=CHECK_INTERVAL_STEPS, 
                                           mode="max", 
                                           monitor="val_f1")
    ...
    evaluator = Trainer(
        ...
        logger=logger,
        limit_val_batches=1.0#50 ## deterministic full dataset (all combinations) -> full. Before, 50 random-sampled batches (bad)
    )
    ...
    ckpt_path = "screensim-noocr-spotify-best-EVAL.ckpt" # or v1
    if os.path.exists(os.path.join(ARTIFACT_DIR, ckpt_path)):
        print("***********************************")
        print(f"Loading from {ckpt_path}")
        print("***********************************\n")
        model = UIScreenEmbedder.load_from_checkpoint(os.path.join(ARTIFACT_DIR, ckpt_path))

    #trainer_optim.fit(model, data_trainrand)
    
    # evaluate perf
    print('\nVALIDATING fitted model..................................\n')
    evaluator.validate(model=model, dataloaders=data_EVALALL.val_dataloader())

    print('\nTESTING fitted model.....................................\n')
    evaluator.test(model=model, dataloaders=data_EVALALL.test_dataloader())
