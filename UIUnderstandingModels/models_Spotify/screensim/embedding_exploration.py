...
class Encoder():
    ...
    def __init__(self):
        ...
        self.spotify_model = UIElementDetector.load_from_checkpoint(
            "screenrecognition_spotify_best_res720.ckpt", 
            val_weights=class_weights, 
            test_weights=class_weights, 
            lr=lr,
        )
        self.spotify_model.hparams.lr = lr
        self.spotify_model.hparams.num_classes = FINETUNE_CLASSES
        mod = self.spotify_model.model.head.classification_head
    ...
    def dataloading(self):
        ''' GET IMAGE '''
        #self.spotify_datamodule = SpotifyUIDataModule(batch_size=1, num_workers=1)
        #self.spotify_test_dataloader = self.spotify_datamodule.test_dataset  #loader()
    ...
    def encoder(self, data_idx: int = 0, show_image: bool = False, load_from_path = None):
        ...
        if load_from_path:
            ...
            images, _ = self.spotify_test_dataloader.__getitem__(data_idx)
            #next(iter(spotify_test_dataloader))
        ...
        feature_embeddings = self.spotify_model.model.backbone(images[0].tensors)
        ...
        #head_outputs = spotify_model.model.head(feature_embeddings)
    ...
    def centerness(self, E):
        center = []
        for e in E:
            center_e = self.spotify_model.model.head.regression_head.bbox_ctrness(e)
            center.append(center_e)
        return center

    def upsample_aggregate(self, E):
        # upsample P4-P7 to P3 shape (256, 80, 40)
        upsample = torch.nn.Upsample(size=(80,40), mode='nearest')
    ...
    def compare_all_enc_combinations(self):
        top = self.spotify_test_dataloader.__len__()
...
if __name__ == '__main__':
    ...
    testpath = "../../downloads/spotify/screenshots/d8b013db-2f8e-47d6-8bab-bb88d81d9f56.png"
    ...
    E = enc.encoder(load_from_path=testpath)
    ...
    cntr = enc.centerness(E)

    for i in range(len(E)):
        print('Embedding shape: ', E[i].shape)
        print('Centerness shape: ', cntr[i].shape)
