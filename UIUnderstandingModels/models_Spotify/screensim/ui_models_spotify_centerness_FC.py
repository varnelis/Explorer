...
class UIScreenEmbedder(pl.LightningModule):
    ...
    def forward_arch5(self, x):
        ...
        # x input has shape (N, 2, 5, 256, 80, 04)
        #x_norm_fpn = self.IN(x[:,0,:,:,:,:])
        #x_norm_center = self.IN(x[:,1,:,:,:,:])
        #x = torch.cat((x_norm_fpn.unsqueeze(1), x_norm_center.unsqueeze(1)), dim=1)

        x = torch.permute(x, (0,2,3,4,5,1)) # shape (N,2,5,256,80,40) --> (N,5,256,80,40,2)
        x = self.linear2(x).squeeze(-1) # squeeze: shape (N,5,256,80,40,1) --> (N,5,256,80,40)
        x = self.relu(x)
        ...
        x7 = self.self_attn(x[:,4,:,:,:]).unsqueeze(1) # each has shape (N,1,256,80,40)
        x = torch.cat((x3,x4,x5,x6,x7),dim=1) # shape (N,5,256,80,40)

        x = torch.permute(x, (0,2,3,4,1))
        x = self.linear5(x)
        x = x.squeeze(-1) # shape (N,256,80,40,1) --> (N,256,80,40)
        x = self.relu(x)

        x = self.conv(x) # shape (N,256,80,40) --> (N,1,80,40)
        
        x = torch.flatten(x, 1) # shape (N,3200) final embedding
    ...
    def forward_p3center3_arch5(self, x):
        x = x[:,:,0,:,:,:] # shape (N,2,256,80,40)
        x = torch.permute(x, (0,2,3,4,1))
        x = self.linear2(x).squeeze(-1) # shape (N,256,80,40)
        x = self.relu(x)
        x = self.self_attn(x)
        x = self.relu(x)
        x = self.conv(x)
    ...
    def get_inputs_from_batch(self, batch):
        ...
        center1 = torch.broadcast_to(center1, (batch_size, 5, 256, 80, 40))
