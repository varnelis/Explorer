...
class SpotifySimilarityDataset(torch.utils.data.IterableDataset):
    def __init__(self, 
    ...
    def sample_same_screen(self):
        try:
            ...
            #ocr1 = torch.tensor(self.uuid2ocr[random_uuid_1][1])
            #ocr2 = torch.tensor(self.uuid2ocr[random_uuid_2][1])
    ...
    def sample_different_screen(self): # different domain
        try:
            sampled_domains = random.sample(list(self.domain_map), 2)        ################### Sample different even in single-example state groups (previously ignored)
            ...
            # ocr embeddings from Sentence-BERT (Sentence Transformers)
            #ocr1 = torch.tensor(self.uuid2ocr[uuid1][1])
            #ocr2 = torch.tensor(self.uuid2ocr[uuid2][1])
...
class SpotifySimilarityDataModule(pl.LightningDataModule):
    def __init__(self, batch_size=16, num_workers=2, split_file="../../metadata/screensim/train_split_spotify_screensim.json"):
        super(SpotifySimilarityDataModule, self).__init__()
        self.batch_size = batch_size
        self.num_workers = num_workers
        self.split_file = split_file
        
        self.train_dataset = SpotifySimilarityDataset(split_file=split_file)
        self.val_dataset = SpotifySimilarityDataset(split_file="../../metadata/screensim/val_split_spotify_screensim.json")
        self.test_dataset = SpotifySimilarityDataset(split_file="../../metadata/screensim/test_split_spotify_screensim.json")
