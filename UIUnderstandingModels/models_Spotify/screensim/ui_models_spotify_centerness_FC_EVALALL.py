...
class UIScreenEmbedder(pl.LightningModule):
    ...
    def get_inputs_from_batch(self, batch):
        ...
        #labels, fpn1, fpn2, center1, center2 = batch ##### OCR inputs disabled -- see & change dataset file
        ...
        center1 = torch.broadcast_to(center1, (batch_size, 5, 256, 80, 40)) ##### Shape of Spotify embeddings is 256,80,40 -- (Khan was 256,40,44)
        center2 = torch.broadcast_to(center2, (batch_size, 5, 256, 80, 40))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        #print(f'Val Step: batch_idx = {batch_idx}, len batch = {len(batch)}')
    ...
    def validation_epoch_end(self, outputs):
        ...
        accuracy = torch.sum(all_labels.detach().cpu() == all_outs.detach().cpu()).item() / len(all_labels.detach().cpu())
        
        #print(f'Number of validation samples: {len(outputs)}')
        #print('Outputs: ', outputs)
        print(f'Val F1: {score} || Val Acc: {accuracy} || Num Val: {len(outputs)}')

        if self.hparams.lambda_ocr == 0:
            metrics = {'val_f1': score, 'val_acc': accuracy}
            self.log_dict(metrics, sync_dist=True)
        else:
            '''
    ...
    def test_step(self, batch, batch_idx):
        ...
        #print(f'Test Step: batch_idx = {batch_idx}, len batch = {len(batch)}')
    ...
    def test_epoch_end(self, outputs):
        ...
        #print(f'Number of test samples: {len(outputs)}')
        #print('Outputs: ', outputs)

        if self.hparams.lambda_ocr == 0:
            metrics = {'test_f1': score, 'test_acc': accuracy}
            self.log_dict(metrics, sync_dist=True)
        else:
            '''
            all_outs_uda = torch.cat([o[2] for o in outputs], dim=0)
